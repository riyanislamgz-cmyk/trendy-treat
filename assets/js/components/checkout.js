...
function renderCheckout() {
    ...
    const total = DB.cartTotal() + (DB.cartTotal() >= DB.freeThreshold() ? 0 : DB.shipping());
    return `
    <section class="checkout-section">
        ...
        <div class="checkout-form" id="checkoutForm">
            <div class="checkout-summary">
                <h4>Order Summary</h4>
                ${items.map(i => `<div style="display:flex;justify-content:space-between;font-size:.9rem;padding:4px 0"><span>${i.name} × ${i.qty}</span><span>$${DB.formatPrice(i.qty * i.price)}</span></div>`).join('')}
                <div style="display:flex;justify-content:space-between;font-size:.9rem;padding:4px 0;border-top:1px solid var(--border);margin-top:8px;padding-top:8px"><span>Shipping</span><span>${DB.cartTotal() >= DB.freeThreshold() ? '<span style="color:#2ecc71">FREE</span>' : DB.formatPrice(DB.shipping())}</span></div>
                <div style="display:flex;justify-content:space-between;font-weight:700;font-size:1.2rem;color:var(--primary);padding-top:12px;border-top:2px solid var(--primary);margin-top:12px"><span>Total</span><span>$${DB.formatPrice(total)}</span></div>
            </div>
            ...
            <button class="btn btn-primary btn-block" onclick="placeOrder()" style="padding:16px;font-size:1.1rem">
                <i class="fas fa-check-circle"></i> Confirm Order — $${DB.formatPrice(total)}
            </button>
